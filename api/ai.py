...
@api_blueprint.route("/healthtok", methods=["GET"])
def health_check():
    return jsonify({"status": "healthy", "service": "TOK Essay Evaluator"}), 200


# Extended Essay evaluation endpoint
@api_blueprint.route("/eecheck", methods=["POST"])
@auth_required
def eecheck():
    try:
        if "file" not in request.files:
        ...
        completion = openai_client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert IB Extended essay evaluator. Provide detailed, constructive feedback following IB assessment criteria. Always respond with valid JSON.",
                },
                {"role": "user", "content": prompt},
            ],
            max_tokens=4096,
...
@api_blueprint.route("/healthee", methods=["GET"])
def health_check_ee():
    return jsonify({"status": "healthy", "service": "Extended Essay Evaluator"}), 200